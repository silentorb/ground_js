...
(function (Ground) {
    var Database = (function () {
        ...
        Database.prototype.add_non_trellis_tables_to_database = function (tables, ground) {
            ...
            var promises = MetaHub.map_to_array(non_trellises, function (table) { return _this.add_table_to_database(table, ground); });
...
(function (Ground) {
    var Query = (function () {
        function Query(trellis, base_path) {
            if (base_path === void 0) { base_path = null; }
            this.joins = [];
            this.post_clauses = [];
        ...
        Query.prototype.get_fields_and_joins = function (properties, include_primary_key) {
            if (include_primary_key === void 0) { include_primary_key = true; }
            var name, fields = [];
            var trellises = {};
        ...
        Query.prototype.get_primary_key_value = function () {
            var _this = this;
            var filters = this.property_filters.filter(function (filter) { return filter.property == _this.trellis.primary_key; });
            if (filters.length > 0)
        ...
        Query.prototype.get_reference_object = function (row, property) {
            ...
            return query.run().then(function (rows) { return rows[0]; });
        };
        ...
        Query.prototype.process_row = function (row) {
            ...
            var links = this.trellis.get_all_links(function (p) { return !p.is_virtual; });
            ...
            return when.all(promises).then(function () { return _this.ground.invoke(_this.trellis.name + '.queried', row, _this); }).then(function () { return row; });
        };
        ...
        Query.prototype.run_core = function () {
            ...
            if (this.properties && Object.keys(this.properties).length > 0) {
                ...
                properties = MetaHub.map(this.properties, function (property, key) { return properties[key]; });
            }
            else {
                properties = this.trellis.get_all_properties();
            }
            var tree = this.trellis.get_tree();
            var promises = tree.map(function (trellis) { return _this.ground.invoke(trellis.name + '.query', _this); });
        ...
        Query.prototype.run = function () {
            ...
            var properties = this.trellis.get_all_properties();
            return this.run_core().then(function (rows) { return when.all(rows.map(function (row) { return _this.process_row(row); })); });
        };
        Query.get_identity_sql = function (property, cross_property) {
            if (cross_property === void 0) { cross_property = null; }
            if (cross_property) {
...
(function (Ground) {
    var Delete = (function () {
        ...
        Delete.prototype.delete_child = function (link, id, depth) {
            ...
            console.log('id', id);
            return query.run(null).then(function (result) { return when.all(result.objects.map(function (object) { return _this.run_delete(other_trellis, object, depth + 1); })); });
        };
...
(function (Ground) {
    ...
    var Core = (function (_super) {
        ...
        Core.prototype.export_schema = function () {
            return {
                trellises: MetaHub.map(this.trellises, function (trellis) { return trellis.export_schema(); })
            };
        };
...
(function (Ground) {
    var Record_Count = (function (_super) {
        ...
        function Record_Count(ground, parent, property_name, count_name) {
            ...
            this.listen(ground, this.child.name + '.created', function (seed, update) { return _this.count(seed); });
            this.listen(ground, this.child.name + '.deleted', function (seed, update) { return _this.count(seed); });
        }
        Record_Count.prototype.count = function (seed) {
            ...
            return this.child.assure_properties(seed, [back_reference.name]).then(function (seed) {
                ...
                return _this.ground.db.query(sql).then(function () { return _this.invoke('changed', parent_key); });
            });
        };
        return Record_Count;
    })(MetaHub.Meta_Object);
    ...
    var Join_Count = (function (_super) {
        ...
        function Join_Count(ground, property, count_name) {
            ...
            this.listen(ground, table_name + '.created', function (seed, property) { return _this.count(seed, property); });
            this.listen(ground, table_name + '.removed', function (seed, property) { return _this.count(seed, property); });
        }
        Join_Count.prototype.count = function (seed, property) {
            ...
            return property.parent.assure_properties(seed, [key_name]).then(function (seed) {
                ...
                return _this.ground.db.query(sql).then(function () { return _this.invoke('changed', key); });
            });
        };
        return Join_Count;
    })(MetaHub.Meta_Object);
    ...
    var Multi_Count = (function (_super) {
        ...
        function Multi_Count(ground, trellis, count_name, sources) {
            ...
            this.count_name = count_name;
            this.count_fields = sources.map(function (c) { return c['count_name']; });
            for (var i in sources) {
                this.listen(sources[i], 'changed', function (key) { return _this.count(key); });
            }
        }
        Multi_Count.prototype.count = function (key) {
            ...
            return this.ground.db.query(sql).then(function () { return _this.invoke('changed', key); });
        };
        return Multi_Count;
    })(MetaHub.Meta_Object);
        ...
        Logic.load_constraint = function (ground, source, scope) {
            if (source.expression.type == 'function') {
                ...
                if (func.name == 'count') {
                    ...
                    var sources = func.arguments.map(function (x) { return scope.symbols[x.path]; });
        ...
        Logic.load_constraint2 = function (ground, source, scope) {
            if (source.expression.type == 'function') {
                ...
                if (func.name == 'count') {
                    ...
                    var sources = func.inputs.map(function (x) { return scope.get_constraint(x.path[0]); });
...
(function (Ground) {
    ...
    var Query_Builder = (function () {
        ...
        Query_Builder.prototype.run_single = function (user, query_result) {
            if (query_result === void 0) { query_result = null; }
            return this.run(user, query_result).then(function (result) { return result.objects[0]; });
        };
    ...
    var Query_Runner = (function () {
        ...
        Query_Runner.get_reference_object = function (row, property, source, query_result) {
            ...
            if (!value)
                return when.resolve(value);
            query.add_key_filter(value);
            return query.run(query_result.user, query_result).then(function (result) { return result.objects[0]; });
        };
        ...
        Query_Runner.prototype.process_row_step_one = function (row, source, query_result, parts) {
            ...
            var trellis = type_property && row[type_property.name] ? this.ground.sanitize_trellis_argument(row[type_property.name]) : source.trellis;
            if (trellis != source.trellis) {
                ...
                return query.run_single(query_result.user, query_result).then(function (row) { return _this.process_row_step_two(row, source, trellis, query_result, parts); });
            }
            else {
        ...
        Query_Runner.prototype.process_row_step_two = function (row, source, trellis, query_result, parts) {
            ...
            var sequence = require('when/sequence');
            return when.all(promises).then(function () { return sequence(cache.tree.map(function (trellis) { return function () { return _this.ground.invoke(trellis.name + '.queried', row, _this, query_result); }; })); }).then(function () { return replacement === undefined ? row : replacement; });
        };
        ...
        Query_Runner.prototype.query_link_property = function (seed, property, source, query_result) {
            ...
            switch (relationship) {
                ...
                case 2 /* one_to_many */:
                case 3 /* many_to_many */:
                    return Query_Runner.get_many_list(seed, property, relationship, source, query_result).then(function (result) { return result ? result.objects : []; });
                    break;
            }
            throw new Error('Could not find relationship: ' + relationship + '.');
        };
        Query_Runner.prototype.prepare = function (query_id) {
            ...
            if (this.row_cache)
                return when.resolve(this.row_cache);
            var tree = source.trellis.get_tree().filter(function (t) { return _this.ground['has_event'](t.name + '.query'); });
            var promises = tree.map(function (trellis) { return function () { return _this.ground.invoke(trellis.name + '.query', source); }; });
            if (this.ground['has_event']('*.query'))
                promises = promises.concat(function () { return _this.ground.invoke('*.query', source); });
            var is_empty = false;
            if (source.filters) {
        ...
        Query_Runner.prototype.render = function (parts) {
            ...
            var source = this.source;
            return this.ground.invoke(source.trellis.name + '.query.sql', parts, source).then(function () { return source.type == 'union' ? _this.render_union(parts) : when.resolve({ sql: _this.renderer.generate_sql(parts, source), queries: [] }); }).then(function (render_result) {
