...
(function (Ground) {
    var Query_Runner = (function () {
        function Query_Runner(source) {
            ...
            this.renderer = new Ground.Query_Renderer(this.schema);
        }
        ...
        Query_Runner.create_sub_query = function (trellis, property, source) {
            var query = new Ground.Query_Builder(trellis, source.schema);
        ...
        Query_Runner.prototype.get_inherited_trellis = function (row, trellis) {
            ...
            return type_property && row[type_property.name] ? this.schema.sanitize_trellis_argument(row[type_property.name]) : trellis;
        };
        Query_Runner.prototype.query_inherited_row = function (row, source, trellis, query_result) {
            ...
            var query = new Ground.Query_Builder(trellis, this.schema);
        ...
        Query_Runner.prototype.process_row_step_two = function (row, source, trellis, query_result, parts) {
            ...
            return when.all(promises).then(function () { return sequence(cache.tree.map(function (trellis) { return function () { return _this.schema.invoke(trellis.name + '.queried', row, _this, query_result); }; })); }).then(function () { return replacement === undefined ? row : replacement; });
        };
        ...
        Query_Runner.prototype.prepare = function (query_id) {
            ...
            var tree = source.trellis.get_tree().filter(function (t) { return _this.schema['has_event'](t.name + '.query'); });
            var promises = tree.map(function (trellis) { return function () { return _this.schema.invoke(trellis.name + '.query', source); }; });
            if (this.schema['has_event']('*.query'))
                promises = promises.concat(function () { return _this.schema.invoke('*.query', source); });
            var is_empty = false;
            if (source.filters) {
        ...
        Query_Runner.prototype.render = function (parts) {
            ...
            var source = this.source;
            return this.schema.invoke(source.trellis.name + '.query.sql', parts, source).then(function () { return source.type == 'union' ? _this.render_union(parts) : when.resolve({ sql: _this.renderer.generate_sql(parts, source), queries: [] }); }).then(function (render_result) {
                var sql = render_result.sql;
                sql = sql.replace(/\r/g, "\n");
                if (_this.schema.log_queries)
                    console.log('\nquery', sql + '\n');
                return {
                    sql: sql,
                    parts: parts,
        ...
        Query_Runner.prototype.run = function (query_result) {
            ...
            return this.prepare().then(function (parts) { return _this.render(parts); }).then(function (render_result) {
                if (!render_result.sql)
                    return when.resolve([]);
                return _this.schema.db.query(render_result.sql).then(function (rows) {
                    var result = {
                        objects: rows
                    };
        ...
        Query_Runner.prototype.paging = function (render_result, result) {
            ...
            if (this.schema.log_queries)
                console.log('\nquery', sql + '\n');
            return this.schema.db.query_single(sql).then(function (count) {
