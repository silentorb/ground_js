...
module mining {
  ...
  export class Query_Runner {
    ...
    private static get_many_list(seed, property:landscape.Property, relationship:landscape.Relationships, source:Query_Builder, query_result:Query_Result, miner:Miner):Promise {
...
//        query.add_filter(other_property.name, id)
    ...
    private static get_reference_object(row, property:landscape.Property, source:Query_Builder, query_result:Query_Result, miner:Miner) {
...
//          properties: source.trellis.get_all_properties()
    ...
    get_inherited_trellis(row, trellis:landscape.Trellis):landscape.Trellis {
      var type_property = trellis.type_property

      return type_property && row[type_property.name]
        ? this.miner.schema.get_trellis(row[type_property.name])
        : trellis
    }

    query_inherited_row(row, source:Query_Builder, trellis:landscape.Trellis, query_result:Query_Result):Promise {
...
//        .then(()=> this.schema.invoke(trellis.name + '.queried', row, this))
    ...
    process_reference_children(child, query:Query_Builder, query_result:Query_Result):Promise {
      ...
      for (name in query.subqueries) {
        ...
        promises.push(()=>
            property.type == 'list'
              ? Query_Runner.get_many_list(child, property, property.get_relationship(), subquery, query_result, this.miner)
              .then((result)=> {
    ...
    query_link_property(seed, property, source:Query_Builder, query_result:Query_Result):Promise {
      ...
      switch (relationship) {
        ...
        case landscape.Relationships.many_to_many:
          return Query_Runner.get_many_list(seed, property, relationship, source, query_result, this.miner)
    ...
    prepare(query_id:number = undefined):Promise {
      ...
      var tree = source.trellis.get_tree().filter((t)=> this.miner.messenger.has_event(t.name + '.query'))
      var promises = tree.map((trellis:landscape.Trellis) => ()=> this.miner.messenger.invoke(trellis.name + '.query', source))
    ...
    render_union(parts):Promise {
      ...
      var promises = this.source.queries.map((query)=> ()=> {
        var runner = new Query_Runner(query, this.miner)
