...
//  }
  ...
  export class Query_Builder {
    ...
    public static create(schema:landscape.Schema, source = null):Query_Builder {
    ...
    add_filter(path:string, value = null, operator:string = '=') {
      ...
      if (path.indexOf('.') === -1) {
        ...
        if (!filter.property) {
          throw new InputError('Invalid filter path.  landscape.Trellis ' + this.trellis.name
    ...
    create_runner(miner:Miner):Query_Runner {
      return new Query_Runner(this, miner)
    }
    ...
    run(user, miner:Miner, query_result:Query_Result = null):Promise {
      if (!query_result)
...
//console.log('query-count', this.trellis.name, query_result.query_count)
      ...
      var runner = new Query_Runner(this, miner)
//      console.log('filters', this.filters)
      return runner.run(query_result)
    }

    run_single(user, miner:Miner, query_result:Query_Result = null):Promise {
      return this.run(user, miner, query_result)
